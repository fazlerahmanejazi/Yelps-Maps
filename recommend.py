...
def find_centroid(cluster):
    ...
    restaurants."""
    latitute=[restaurant_location(x)[0] for x in cluster]
    longitude=[restaurant_location(x)[1] for x in cluster]
    return [mean(latitute), mean(longitude)]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    f=max(feature_fns, key=lambda x: find_predictor(user, reviewed, x)[1])
    return find_predictor(user, reviewed, f)[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for x in restaurants:
        if restaurant_name(x) in reviewed:
            predicted_rating.update({restaurant_name(x):user_rating(user, restaurant_name(x))})
        else:
            predicted_rating.update({restaurant_name(x):predictor(x)})
